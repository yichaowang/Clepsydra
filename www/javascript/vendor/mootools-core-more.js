...
var Tips = new Class({
	...
	build: function(el){ 
		if (el.href && el.tagName == 'a') {
			el.store('myTitle', el.href.replace('http://', ''))
		}else{
			el.store('myTitle', (el.rel || false));
		} 
		if (el.title){
		...
		if ( (el.retrieve('myTitle')) && (el.retrieve('myTitle').length > this.options.maxTitleChars)){
			el.store('myTitle', el.retrieve('myTitle').substr(0, this.options.maxTitleChars - 1) + "&hellip;");
		}
